...
type Split<Value extends string> = Value extends `${infer LValue}/${infer RValue}` ? Filter<Value> | Split<RValue> : Filter<Value>;

export type GetPathParams<T extends string> = { [key in Split<T>]: string | number };
